...
from lib.transform import Rescale, RandomCrop, ToTensor
from lib.network import Net, Net1, Net2, Net3, Net4, Net5, Net6, Net7
...
if __name__ == "__main__":
    ...
    data_transform = transforms.Compose(
        [Rescale(RESCALE_SIZE), RandomCrop(RDM_CROP_SIZE), ToTensor()]
    )
    ...
    print(
        ">>> %d pictures for training\n>>> %d pictures for testing"
    ...
    for epoch in range(EPOCH_NBR):
        ...
        for i, data in enumerate(train_loader, 0):
            ...
            if (
                i % len(train_loader) == len(train_loader) - 1
            ):  # print every 200 mini-batches
                print(
                    ">>> progress "
                    + progress_bar
                    + " %d/%d mean loss: %.3f"
                    % (epoch + 1, EPOCH_NBR, running_loss / len(train_loader)),
                    end="\r",
                )
                running_loss = 0.0
            else:
                print(
                    ...
                    + progress_bar
                    + " %d/%d mean loss: %.3f"
                    % (epoch + 1, EPOCH_NBR, running_loss / (i + 1)),
                    end="\r",
                )
    ...
    class_correct = list(0.0 for i in range(8))
    class_total = list(0.0 for i in range(8))
    with torch.no_grad():
    ...
    full_test = (
        True
        if "y"
        == input("Do you want to do a test over the whole database? (y/n): ").lower()
        else False
    )
        ...
        with torch.no_grad():
            for k, data in enumerate(dataloader, 0):
                ...
                print(
                    ">>> progress "
                    + progress_bar
                    + " %d/%d" % ((k + 1) * BATCH_SIZE, len(dataloader) * BATCH_SIZE),
                    end="\r",
                )
    ...
    save = (
        True
        if "y" == input("Do you want to save this network? (y/n): ").lower()
        else False
    )
